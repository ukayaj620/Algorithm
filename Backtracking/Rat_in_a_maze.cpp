...
int main()
{
	...
	mazeGenerator( maze, &yStart, &xStart );
	
	x = xStart; // starting col
	y = yStart; // starting row
	
	mazeTraversal( maze, xStart, yStart, y, x, RIGHT );
...
void mazeTraversal( char maze[][ 12 ], const int xCoord, const int yCoord,int row, int col, int direction )
{
	...
	if ( coordsAreEdge( row, col ) && row != yCoord && col != xCoord ) {
		cout << "Maze successfully exited!\n\n";
		return; // maze is complete
	}
	else if ( row == yCoord && col == xCoord && flag ) {
